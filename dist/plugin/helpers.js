 ...
 * server to reject.
 */
export function toPromptInputParts(parts) {
    const out = [];
...
export const CONFIG_SIDECAR = ['.opencode', 'escalator.json'];
/**
 ...
 */
export function loadUserConfig(directory, options) {
    ...
    if (options !== undefined && options !== null) {
        if (typeof options !== 'object' || Array.isArray(options)) {
            throw new Error('opencode-model-escalator: plugin `options` must be an object when supplied (CFG-2).');
        }
    ...
    try {
        ...
        throw new Error(`opencode-model-escalator: failed to read config sidecar at ${sidecarPath}: ${err.message} (CFG-2).`);
    }
    let parsed;
    try {
        ...
        throw new Error(`opencode-model-escalator: config sidecar at ${sidecarPath} is not valid JSON: ${err.message} (CFG-2).`);
    }
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        throw new Error(`opencode-model-escalator: config sidecar at ${sidecarPath} must contain a JSON object (CFG-2).`);
    }
    return parsed;
}
...
function hasFailureEvidence(r) {
    if (r.error !== undefined && r.error !== null && r.error !== false)
        return true;
    if (r.success === false || r.ok === false || r.changed === false)
        return true;
    if (r.failed === true || r.aborted === true)
        return true;
    if (r.cancelled === true || r.canceled === true)
        return true;
    if (typeof r.status === 'string' && RE_FAILURE_STATUS.test(r.status.trim())) {
        return true;
    }
    for (const k of ['exit', 'exitCode']) {
        const v = r[k];
 ...
 */
export function toolDidMutate(output) {
    // A non-object result carries no status channel ⇒ trust the run (rule 4).
    if (output === null || typeof output !== 'object')
        return true;
    const o = output;
    const meta = o.metadata !== null && typeof o.metadata === 'object'
        ? o.metadata
        : undefined;
    ...
    if (o.success === true || o.ok === true || o.changed === true)
        return true;
    if (positiveEdits(o.edits))
        return true;
    if (meta !== undefined) {
        if (meta.success === true || meta.changed === true || meta.ok === true)
            return true;
        if (positiveEdits(meta.edits))
            return true;
    }
 ...
 */
export function readExitCode(metadata) {
    if (metadata === null || typeof metadata !== 'object')
        return undefined;
    const raw = metadata.exit;
 ...
 */
export function errorToSignal(error) {
    if (error === null || typeof error !== 'object')
        return {};
    const e = error;
    const data = e.data !== null && typeof e.data === 'object'
        ? e.data
        : {};
    const signal = {};
