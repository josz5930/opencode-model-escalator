...
import { classifyFailure, failureFingerprint, isOversizedOutput, looksLikeTestCommand, } from './detection.js';
import { clearFailureState, initialState, markCodeChanged, recordFailure, } from './counter.js';
const TERMINAL_STOP_MESSAGE = 'Max escalation reached — automation stopped';
const DISPATCH_FAILED_MESSAGE = 'Escalation dispatch failed — automation stopped';
const INFRA_EXHAUSTED_MESSAGE = 'Infrastructure failure persists — automatic retries stopped';
...
function redactCommand(cmd) {
    ...
    const masked = cmd
        ...
        .replace(/((?:^|\s)(?:[A-Za-z0-9_]*(?:TOKEN|KEY|SECRET|PASSWORD|PASSWD|PWD|AUTH|CREDENTIAL|BEARER)[A-Za-z0-9_]*)\s*[:=]\s*)("[^"]*"|'[^']*'|\S+)/gi, '$1<redacted>')
        // Space-separated credential flags: `--token SECRET` / `-p SECRET`.
        .replace(/((?:^|\s)(?:--[A-Za-z0-9-]*(?:token|key|secret|password|passwd|pwd|auth|credential|bearer)[A-Za-z0-9-]*|-p|-P)\s+)("[^"]*"|'[^']*'|\S+)/gi, '$1<redacted>')
...
export function createEscalator(args) {
    ...
    function applyStuck(state, next) {
    ...
    function log(entry) {
        if (!config.debug && entry.level === 'debug')
            return;
     ...
     */
    async function escalate(sessionID, state) {
        ...
        try {
            ...
            if (state.terminated || !state.enabled)
                return;
     ...
     */
    async function latchInfraStop(sessionID, state, info) {
    ...
    async function handleCategoryA(sessionID, state, info) {
        ...
        if (state.terminated || state.infraStopped)
            return;
        ...
        try {
            if (state.terminated || state.infraStopped || !state.enabled)
                return;
            if (state.categoryARetries >= config.max_infra_retries) {
                await latchInfraStop(sessionID, state, {
                    statusLabel: info.statusLabel,
            ...
            const delayMs = Math.min(config.infra_retry_cooldown_ms * 2 ** (state.categoryARetries - 1), MAX_TIMER_MS);
     ...
     */
    async function rebindToCheapest(sessionID, state, expectedTaskId) {
        const release = await acquire(state);
        try {
            if (state.terminated || !state.enabled)
                return;
    ...
    async function onTestResult(input) {
        ...
        const hasMarker = config.fingerprint.failure_markers.some((m) => m.length > 0 && output.includes(m));
        const isFailure = (typeof input.exitCode === 'number' && input.exitCode !== 0) || hasMarker;
        if (!isFailure) {
            log({
                level: 'debug',
        ...
        const result = recordFailure(state, fingerprint, threshold, config.require_code_change_between_failures);
    ...
    function beginNewTask(state, input) {
    ...
    async function onChatMessage(input) {
        ...
        if (pending !== undefined) {
            const idsComparable = pending.taskId !== undefined && input.taskId !== undefined;
    ...
    async function onSessionError(input) {
        if (!isSessionID(input.sessionID))
            return;
        const state = ensure(input.sessionID, input.now);
        if (!state.enabled)
        ...
        const text = `${input.status !== undefined ? `HTTP ${input.status} ` : ''}${input.message ?? ''}`;
        ...
        await handleCategoryA(input.sessionID, state, {
            statusLabel: input.status !== undefined ? `HTTP ${input.status}` : 'session-error',
        });
    }
    function statusOf(sessionID, state) {
        const status = {
            sessionID,
            ...
            config: {
                ...
                model_thresholds: config.models.map((_, i) => thresholdForStage(config, i)),
                require_code_change_between_failures: config.require_code_change_between_failures,
        ...
        if (state.previousFailure !== undefined) {
            status.previousFailure = state.previousFailure.slice(0, FINGERPRINT_DISPLAY_LEN);
        }
    ...
    function control(sessionID, action, now) {
    ...
    function resultsGenerationOf(sessionID) {
        const s = sessions.get(sessionID);
