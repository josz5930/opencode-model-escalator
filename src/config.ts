...
const RE_NESTED_QUANTIFIER = new RegExp(
  `\\((?:[^()\\\\]|\\\\.)*${RE_QUANTIFIER}[^()]*\\)\\s*${RE_QUANTIFIER}`,
);

function assertSafeRegexSource(source: string): void {
  if (source.length === 0) {
    throw new Error(
      'opencode-model-escalator: a `retry_on_patterns` entry is an empty string — an empty regex matches everything and would classify every failure as Category A (CFG-2).',
    );
  }
  if (RE_NESTED_QUANTIFIER.test(source)) {
    throw new Error(
      `opencode-model-escalator: \`retry_on_patterns\` entry "${source}" contains a nested-quantifier construct prone to catastrophic backtracking (ReDoS); rewrite it without a quantifier inside a quantified group, or match literally (CFG-2).`,
    );
  }
}
 ...
 */
function assertNotMatchAll(source: string): void {
  try {
    const re = new RegExp(source, 'i');
    if (re.test('')) {
      throw new Error(
        `opencode-model-escalator: \`retry_on_patterns\` entry "${source}" matches the empty string and would classify every failure as Category A (CFG-2).`,
      );
    }
    if (re.test(MATCH_ALL_PROBE)) {
      throw new Error(
        `opencode-model-escalator: \`retry_on_patterns\` entry "${source}" is over-broad — it matches ordinary non-infrastructure text and would classify nearly every failure as Category A, disabling capability escalation; write a more specific pattern (CFG-2).`,
      );
    }
  } catch (err) {
...
function assertBoolean(value: unknown, key: string): void {
  if (value !== undefined && typeof value !== 'boolean') {
    throw new Error(
      `opencode-model-escalator: \`${key}\` must be a boolean when supplied; got ${String(
        value,
      )} (CFG-2).`,
    );
  }
...
function assertIntInRange(
  ...
  if (
    ...
    throw new Error(
      `opencode-model-escalator: \`${key}\` must be an integer in [${min}, ${max}] when supplied; got ${String(
        value,
      )} (CFG-2).`,
    );
  }
...
function assertStringArray(value: unknown, key: string): void {
  ...
  if (!Array.isArray(value)) {
    throw new Error(
      `opencode-model-escalator: \`${key}\` must be an array of strings when supplied (CFG-2).`,
    );
  }
  for (const el of value) {
    if (typeof el !== 'string' || el.trim().length === 0) {
      throw new Error(
        `opencode-model-escalator: \`${key}\` entries must be non-empty strings; got ${String(
          el,
        )} (CFG-2).`,
      );
    }
...
function trimmedStringArray(value: string[]): string[] {
...
export function resolveConfig(user: UserConfig | undefined): EscalatorConfig {
  ...
  if (u.mutating_tools !== undefined) {
    if (!Array.isArray(u.mutating_tools) || u.mutating_tools.length === 0) {
      throw new Error(
        'opencode-model-escalator: `mutating_tools` must be a non-empty array of tool names when supplied (CFG-2).',
      );
    }
  ...
  assertBoolean(
    u.require_code_change_between_failures,
  ...
  assertIntInRange(
    u.infra_retry_cooldown_ms,
    'infra_retry_cooldown_ms',
    0,
    MAX_TIMER_MS,
  );

  if (
    u.shell_tool_name !== undefined &&
    (typeof u.shell_tool_name !== 'string' || u.shell_tool_name.trim().length === 0)
  ) {
    throw new Error(
      'opencode-model-escalator: `shell_tool_name` must be a non-empty string when supplied (CFG-2).',
    );
  }
  const shellName =
    typeof u.shell_tool_name === 'string'
      ? u.shell_tool_name.trim()
      : DEFAULTS.shell_tool_name;
  const mutatingTools = trimmedStringArray(
    (u.mutating_tools ?? DEFAULTS.mutating_tools) as string[],
  );
  ...
  // loudly rather than accept an unusable, misleading combination.
  {
    if (mutatingTools.includes(shellName)) {
      throw new Error(
        `opencode-model-escalator: \`shell_tool_name\` ("${shellName}") must not also appear in \`mutating_tools\` — the shell tool is the test-runner channel and cannot double as a mutation signal (CFG-2).`,
      );
    }
  }
  ...
  if (u.fingerprint !== undefined) {
    if (!isPlainObject(u.fingerprint)) {
      throw new Error(
        'opencode-model-escalator: `fingerprint` must be an object when supplied (CFG-2).',
      );
    }
    const fpKeys: (keyof FingerprintConfig)[] = [
      'normalize_durations',
    ...
    if (u.fingerprint.project_root !== undefined) {
      ...
      if (typeof pr !== 'string') {
        throw new Error(
          'opencode-model-escalator: `fingerprint.project_root` must be a string when supplied (CFG-2).',
        );
      }
      ...
      if (trimmed.length > 0) {
        ...
        if (!isAbsolute || isBareRoot || trimmed.length < 2) {
          throw new Error(
            `opencode-model-escalator: \`fingerprint.project_root\` must be an absolute, non-degenerate path when supplied; got "${pr}" (a bare or relative root would scrub distinct failures into one fingerprint) (CFG-2).`,
          );
        }
      }
    }
  ...
  if (u.retry_on_errors !== undefined) {
    ...
    for (const code of u.retry_on_errors) {
      if (
        ...
        throw new Error(
          `opencode-model-escalator: \`retry_on_errors\` entries must be HTTP status codes in [100, 599]; got ${String(
            code,
          )} (CFG-2).`,
        );
      }
  ...
  if (u.retry_on_patterns !== undefined) {
    ...
    if (u.retry_on_patterns.length > MAX_RETRY_PATTERNS) {
      throw new Error(
        `opencode-model-escalator: \`retry_on_patterns\` may contain at most ${MAX_RETRY_PATTERNS} entries; got ${u.retry_on_patterns.length} (CFG-2, ReDoS bound).`,
      );
    }
    for (const source of u.retry_on_patterns) {
      ...
      if (source.length > MAX_RETRY_PATTERN_LEN) {
        throw new Error(
          `opencode-model-escalator: a \`retry_on_patterns\` entry exceeds ${MAX_RETRY_PATTERN_LEN} characters (CFG-2, ReDoS bound).`,
        );
      }
  ...
  if (
    !Number.isInteger(topThreshold) ||
    topThreshold < 1 ||
    topThreshold > MAX_SAME_FAILURE_THRESHOLD
  ) {
    throw new Error(
      `opencode-model-escalator: \`same_failure_threshold\` must be an integer in [1, ${MAX_SAME_FAILURE_THRESHOLD}]; got ${String(
  ...
  const models: ModelStage[] = u.models.map((raw, i) => {
    ...
    if (/\s/.test(model)) {
      throw new Error(
        `opencode-model-escalator: models[${i}] model id "${model}" contains whitespace — supply a clean \`provider/model\` id with no surrounding or interior spaces (CFG-2, AC-14).`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `opencode-model-escalator: models[${i}] has an unparseable model id "${model}" — expected non-empty \`provider/model\` (CFG-2, AC-14).`,
      );
    }
    ...
    if (raw.same_failure_threshold !== undefined) {
      ...
      if (
        ...
        throw new Error(
          `opencode-model-escalator: models[${i}].same_failure_threshold must be an integer in [1, ${MAX_SAME_FAILURE_THRESHOLD}]; got ${String(
            t,
          )} (CFG-2).`,
        );
      }
  ...
  const fingerprint: FingerprintConfig = {
    ...
    ...stripUndefined((u.fingerprint ?? {}) as Record<string, unknown>),
  };
  if (u.fingerprint?.failure_markers !== undefined) {
    fingerprint.failure_markers = trimmedStringArray(
      u.fingerprint.failure_markers,
    );
  }
