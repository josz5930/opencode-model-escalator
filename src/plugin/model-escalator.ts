 ...
 */
export const ModelEscalator: Plugin = async ({ client, directory }, options) => {
  ...
  const mutatingTools: ReadonlySet<string> = new Set(config.mutating_tools);
   ...
   */
  async function dispatchLastUserTurn(
    sessionID: string,
    model: string,
  ): Promise<void> {
    ...
    if (messages.error !== undefined) {
      throw new Error(
        `session.messages failed: ${JSON.stringify(messages.error)}`,
      );
    }

    const payload = getLastUserPayload(
      messages.data as SessionMessage[] | undefined,
    );
    if (payload === null) {
    ...
    if (parts.length === 0) {
      throw new Error(
        `no replayable input parts in session ${sessionID}`,
      );
    }
    ...
    if (prompt.error !== undefined) {
      throw new Error(
        `session.promptAsync failed: ${JSON.stringify(prompt.error)}`,
      );
    }
  }
  ...
  const effects: EscalatorEffects = {
    async abort(sessionID) {
      ...
      if (res !== undefined && res !== null && (res as { error?: unknown }).error !== undefined) {
        throw new Error(
          `session.abort failed: ${JSON.stringify((res as { error?: unknown }).error)}`,
        );
      }
    },
    ...
    async retry(sessionID, model, delayMs, ctx) {
      ...
      if (ctx !== undefined && !ctx.isCurrent()) return;
      await dispatchLastUserTurn(sessionID, model);
    },
    ...
    async log(entry: LogEntry) {
      ...
      if (entry.sessionID !== undefined) extra.sessionID = entry.sessionID;
      if (entry.data !== undefined) Object.assign(extra, entry.data);
   ...
   */
  const callEpochs = new Map<string, number>();
  ...
  function tick(sessionID: string | undefined, now: number): void {
    if (isSessionID(sessionID)) escalator.touch(sessionID, now);
    escalator.gc(now);
  }
  ...
  const hooks: Hooks = {
    'tool.execute.before': async (input) => {
      ...
      if (callEpochs.size > MAX_CALL_EPOCHS) {
        ...
        if (oldest !== undefined) callEpochs.delete(oldest);
      }
    },
    ...
    'tool.execute.after': async (input, output) => {
      ...
      if (input.tool === config.shell_tool_name) {
        const command = String(
          (input.args as { command?: unknown })?.command ?? '',
        );
    ...
    'chat.message': async (input) => {
      ...
      const m = (input as { model?: { providerID?: unknown; modelID?: unknown } })
        .model;
      const model =
        m !== undefined &&
    ...
    tool: {
      ...
      model_escalator_control: tool({
        description:
          'Control the model-escalator for the current session: enable, ' +
        ...
        async execute(args, context) {
          ...
          const status = escalator.control(
            context.sessionID,
            args.action,
            Date.now(),
          );
